...
void Shortest_Path(Edge_node *input, int E)
{
    int n = 0;
    while(n<E)
    {
        if(input[n].destination==input[n+1].source)
        {
            if((input[n+1].weight += input[n].weight) < input[n].weight)
            {
                input[n+1].weight += input[n].weight;
            }
        }
        n++;
    }
...
void Dijktras_algo(Edge_node *input, int V, int E)
{
